// lib/modules/housing/screens/energy/energy_list_screen.dart
// Lijst van energiecontracten voor een woning

import 'package:flutter/material.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';
import '../../models/energy_contract_model.dart';
import '../../providers/housing_providers.dart';
import 'energy_detail_screen.dart';

class EnergyListScreen extends ConsumerWidget {
  final String propertyId;

  const EnergyListScreen({
    super.key,
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final contractsAsync = ref.watch(energyContractsProvider(propertyId));
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: const Text('Contract toevoegen'),
      ),
    );
  }
  ...
  Widget _buildContractCard(BuildContext context, EnergyContractModel contract, WidgetRef ref) {
    ...
    return Card(
      ...
      child: ListTile(
        leading: Container(
          ...
          child: Center(
            child: Text(contract.energyType.emoji, style: const TextStyle(fontSize: 24)),
          ),
        ),
        title: Text(
        ...
        subtitle: Text(
          '${contract.energyType.label} • ${contract.contractType.label}',
        ...
        onTap: () {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (_) => EnergyDetailScreen(
                propertyId: propertyId,
                contractId: contract.id,
              ),
            ),
          ).then((_) => ref.invalidate(energyContractsProvider(propertyId)));
        },
      ),
    );
  }
  ...
  Future<void> _addContract(BuildContext context, WidgetRef ref) async {
    ...
    final contractId = await repo.createEnergyContract(
      propertyId: propertyId,
    );
